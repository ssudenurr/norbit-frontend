    ...
    Array.from(
      ...
      0 < e.querySelectorAll(".notification-item").length
        ...
        : (e.querySelector(".view-all") &&
          ...
          (e.innerHTML +=
            '<div class="empty-notification-elem">\t\t\t\t\t\t\t<div class="w-25 w-sm-50 pt-3 mx-auto">\t\t\t\t\t\t\t\t<img class="img-fluid" >\t\t\t\t\t\t\t</div>\t\t\t\t\t\t\t<div class="text-center pb-5 mt-2">\t\t\t\t\t\t\t\t<h6 class="fs-18 fw-semibold lh-base">Hey! You have no any notifications </h6>\t\t\t\t\t\t\t</div>\t\t\t\t\t\t</div>'));
    });
  }
  function G() {
    var e;
...
function getNotificationList() {
  ...
  axios({
    ...
    .then((response) => {
      ...
      showNotificationMessage(responseData)
    })
    .catch((error) => {
...
// getNotificationMessage(19)

async function showNotificationMessage(responseData) {
  const notificationCard = document.getElementById('notification-card');

  responseData.forEach(async notification => {
    const notificationMessage = notification.message;
    const messageDiv = document.createElement('div');
    messageDiv.innerHTML = `
    <div class="text-reset notification-item d-block dropdown-item position-relative" id="notification">
      <div class="d-flex">
        <div class="flex-1">
          <div class="fs-13 text-muted" id="notification-wrapper">
            <p class="mb-1" id="notification-message">${notificationMessage}</p>
          </div>
        </div>
...
{/* <div>
<li>
    <a class="mdi mdi-close delete-button" type="button" style="float: right;" ></a>
</li>
</div> */}
function getNotificationMessage(id) {
  const apiUrl = `${baseUrl}notification/${id}`;
  const token = localStorage.getItem("token");
